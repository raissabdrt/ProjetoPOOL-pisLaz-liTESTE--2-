...
public class MidiaController {
    ...
    @PostMapping
    public ResponseEntity<String> cadastrarMidia(@RequestBody dto.MidiaCadastroRequest request) {
    ...
    @PostMapping("/avaliar")
    public ResponseEntity<String> avaliarMidia(@RequestBody dto.AvaliacaoRequest request) {
        if (request.nota < 0 || request.nota > 10) {
             return ResponseEntity.badRequest().body("Erro: A nota deve estar entre 0 e 10.");
        }
        ...
        if (sucesso) {
            ...
            return ResponseEntity.badRequest().body("Erro: Não foi possível registrar a avaliação.");
        }
    }
}